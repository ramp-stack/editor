...
use crate::editor::viewer::{file_lang, mount_image_obj, FileMode};
...
pub fn setup(cv: &mut Canvas, ed: &Editor) {
    ...
    cv.add_game_object(
        ...
        GameObject::build(&names.bg)
            ...
            .image(tint_overlay(
                4000.0,
                4000.0,
                theme_snap.get("editor_bg").copied().unwrap(),
            ))
            .finish(),
    );
    ...
    cv.add_game_object(names.gutter_bg.clone(), {
        let mut o = GameObject::build(&names.gutter_bg)
            .position(ex, ey)
            .size(cfg_snap.gutter_w, eh)
            .layer(2)
            .image(tint_overlay(
                4000.0,
                4000.0,
                theme_snap.get("gutter_bg").copied().unwrap(),
            ))
            .finish();
