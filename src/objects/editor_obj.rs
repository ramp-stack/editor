...
use crate::components::highlight::{build_gutter_slice, build_colored_text, ParseCache};
...
pub fn setup(cv: &mut Canvas, ed: &Editor) {
    ...
    let (init_text, init_gutter) = {
        ...
        let lines = if st.lines.is_empty() { vec![String::new()] }
                    else { st.lines[..end].to_vec() };
        (
            build_colored_text(&lines, &ed.code_font, &cfg, &theme,
                &file_lang(&init_path), &init_cache, 0),
    ...
    if init_mode == FileMode::Text {
        ...
        if let Some(line) = st.lines.iter().max_by_key(|l| l.chars().count()) {
            let t = build_colored_text(
                std::slice::from_ref(line), &ed.code_font, &cfg, &theme,
                &file_lang(&init_path), &init_cache, 0,
            );
...

    {
        ...
        let mut o = GameObject::build(&bname)
            .position(mx, ey).size(MINIMAP_W, eh).layer(2)
...

    {
        ...
        let mut o = GameObject::build(&vname)
            .position(mx, ey).size(MINIMAP_W, 20.0).layer(5)
            .finish();
...

    {
        ...
        let mut o  = GameObject::build(&thname)
            .position(sb_x, ey).size(SCROLLBAR_W, SCROLLBAR_MIN_THUMB_H).layer(7)
            .image(solid_rect(SCROLLBAR_W, SCROLLBAR_MIN_THUMB_H, quartz::Color(80, 90, 120, 200)))
            .finish();
        o.visible = false;
