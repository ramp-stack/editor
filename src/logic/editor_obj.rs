...
use crate::components::highlight::{build_gutter_slice, build_colored_text, ParseCache, AsyncParser};
use crate::components::image_view;
use crate::components::language::{file_lang, FileMode};
use crate::components::minimap::{self, MINIMAP_W, MINIMAP_LH, MINIMAP_PAD_X};
...
pub fn minimap_line_name(prefix: &str, i: usize) -> String { format!("{}_mm_{}", prefix, i) }
pub fn minimap_bg_name(prefix: &str)              -> String { format!("{}_minimap_bg", prefix) }
pub fn scrollbar_track_name(prefix: &str)         -> String { format!("{}_sb_track", prefix) }
pub fn scrollbar_thumb_name(prefix: &str)         -> String { format!("{}_sb_thumb", prefix) }
...
pub fn outline_rect(w: f32, h: f32, col: quartz::Color, filled: bool) -> quartz::Image {
    ...
    let fill     = if filled { Rgba([col.0, col.1, col.2, 30]) } else { Rgba([0, 0, 0, 0]) };
    for y in 0..ph { for x in 0..pw {
        img.put_pixel(x, y, if x==0||y==0||x==pw-1||y==ph-1 { border } else { fill });
    }}
    quartz::Image { shape: quartz::ShapeType::Rectangle(0.0, (w, h), 0.0), image: Arc::new(img), color: None }
}
...
pub fn solid_rect(w: f32, h: f32, col: quartz::Color) -> quartz::Image {
    ...
    for y in 0..ph { for x in 0..pw { img.put_pixel(x, y, px); } }
    quartz::Image { shape: quartz::ShapeType::Rectangle(0.0, (w, h), 0.0), image: Arc::new(img), color: None }
}
...
pub fn setup(cv: &mut Canvas, ed: &Editor) {
    ...
    cv.add_game_object(names.bg.clone(),
        GameObject::build(&names.bg)
            .position(ex, ey).size(ew, eh).layer(0)
            .image(tint_overlay(4000.0, 4000.0, theme.get("editor_bg").copied().unwrap()))
            .finish(),
    );

    image_view::mount(cv, &ed.id_prefix, &init_mode, &init_path, ex, ey, ew, eh, 1, &ed.code_font);

    let viewport_lines = if eh > cfg.text_y { ((eh - cfg.text_y) / lh).ceil() as usize + 2 } else { 2 };
    ...
    let (init_text, init_gutter) = {
        ...
        (
            build_colored_text(&lines, &ed.code_font, &cfg, &theme, &file_lang(&init_path), &init_cache, 0),
...
pub fn register(cv: &mut Canvas, ed: &Editor) {
...

    {
        ...
        cv.on_mouse_press(move |_cv, _btn, (mx, my)| {
            ...
            let mmx  = ex + ew - MINIMAP_W;
            let sb_x = ex + ew - SCROLLBAR_W;
            if mx < mmx || mx >= sb_x || my < ey || my > ey + eh { return; }
            ...
            let mm_scroll_px = if v_max > 0.0 { (gs / v_max) * max_mm_off } else { 0.0 };
            let click_mm_px  = mm_scroll_px + (my - ey);
            let rect_h_mm    = (viewport_h / lh) * MINIMAP_LH;
            let rect_top_mm  = (click_mm_px - rect_h_mm * 0.5).max(0.0).min((total_mm_h - rect_h_mm).max(0.0));
    ...
    cv.on_update(move |cv| {
        ...
        let code_x = ex + cfg.text_x;
        let code_y = ey + cfg.text_y;
        // code_w is the visible text area: from after the gutter to before the minimap
        let code_w = ew - cfg.text_x - RIGHT_PAD - MINIMAP_W;
        let mmx    = ex + ew - MINIMAP_W;
        let sb_x   = ex + ew - SCROLLBAR_W;
        ...
        let (cur_mode, cur_path, cur_lang, total, cur_row, cur_col, content_version) = {
            ...
            (st.mode.clone(), st.path.clone(), st.lang.clone(),
             st.lines.len(), st.cursor_row, st.cursor_col, st.content_version)
        };
        ...
        if is_image {
            image_view::update(cv, &id_prefix_u, &cur_path, ex, ey, ew, eh, &img_loaded_key_u, &code_font_u);
            ...
            for i in 0..SEL_OVERLAY_COUNT {
                if let Some(o) = cv.get_game_object_mut(&sel_overlay_name(&id_prefix_u, i)) { o.visible = false; }
            }
            if let Some(o) = cv.get_game_object_mut(&scrollbar_track_name(&id_prefix_u)) { o.visible = false; }
            if let Some(o) = cv.get_game_object_mut(&scrollbar_thumb_name(&id_prefix_u)) { o.visible = false; }
            for i in 0..MINIMAP_LINE_POOL {
                if let Some(o) = cv.get_game_object_mut(&minimap_line_name(&id_prefix_u, i)) { o.visible = false; }
            }
            if let Some(o) = cv.get_game_object_mut(&minimap_bg_name(&id_prefix_u)) { o.visible = false; }
            return;
        }
        ...
        if intent != 0.0 { v_scroll_u.get_mut().set_intent(intent); }
        ...
        if h_scroll_u.get().offset > h_max { h_scroll_u.get_mut().offset = h_max; }
        ...
        let visible_lines = if eh > cfg.text_y { ((eh - cfg.text_y) / lh).ceil() as usize + 2 } else { 2 };
        ...
        if *dragging_u.get() {
            if let Some((mx, my)) = cv.mouse_position() {
                ...
                if !st.lines.is_empty() {
                    let row = (((my_c - ey - cfg.text_y + gs) / lh).floor().max(0.0) as usize).min(st.lines.len().saturating_sub(1));
                    let col = (((mx_c - ex - cfg.text_x + hs) / cw).floor().max(0.0) as usize).min(selection::char_len(&st.lines[row]));
                    st.cursor_row = row;
                    st.cursor_col = col;
        ...
        if (new_parse_ready || (slice_moved && !fast_scrolling)) && last_visible > slice_start {
            ...
            let text  = build_colored_text(
                ...
                &code_font_u, &cfg, &theme, &cur_lang, &*cache, slice_start,
            );
            drop(st); drop(cache);
...

        {
            ...
            let overlays = selection::overlay_rects(
                ...
                slice_start, visible_lines, text_top, code_x, hs, cw, lh, cw * 0.5,
            );
            for i in 0..SEL_OVERLAY_COUNT {
                ...
                match overlays.iter().find(|v| v.abs_row == slice_start + i) {
                    ...
                    Some(ovr) => {
                        if (o.size.0 - ovr.width).abs() > 0.5 {
                            o.set_image(tint_overlay(ovr.width.max(1.0), lh.max(1.0), quartz::Color(38, 79, 120, 180)));
                        }
                ...
                *blink_timer_u.get_mut() = bt;
                if bt >= crate::constants::BLINK_RATE_SECS {
                    *blink_timer_u.get_mut() = 0.0;
                    let vis = *cursor_vis_u.get(); *cursor_vis_u.get_mut() = !vis;
                }
            } else {
        ...
        {
            let thumb_h   = if v_max > 0.0 { (viewport_h / (viewport_h + v_max) * eh).max(20.0).min(eh) } else { eh };
            let thumb_top = if v_max > 0.0 { (gs / v_max) * (eh - thumb_h) } else { 0.0 };

            if let Some(o) = cv.get_game_object_mut(&scrollbar_track_name(&id_prefix_u)) {
                o.position = (sb_x, ey); o.size = (SCROLLBAR_W, eh); o.visible = true;
            }

            let needs_resize = (sn.thumb_h - thumb_h).abs() > 0.5;
            if let Some(o) = cv.get_game_object_mut(&scrollbar_thumb_name(&id_prefix_u)) {
                if needs_resize {
                    o.set_image(solid_rect(SCROLLBAR_W, thumb_h, quartz::Color(100, 110, 150, 220)));
...

        {
            ...
            if new_parse_ready || mm_moved || sn.mm_texts_empty {
                let st    = state_u.lock().unwrap();
                let built = minimap::build_minimap_texts(&st.lines[first_mm..last_mm], &theme, &code_font_u);
                drop(st);
                let mut r = rs.get_mut();
                r.mm_texts = built;
                r.mm_first = first_mm;
            ...
            for slot in 0..MINIMAP_LINE_POOL {
                ...
                let Some(o) = cv.get_game_object_mut(&lname) else { continue };
                if slot >= visible_mm || slot >= mm_texts_len { o.visible = false; continue; }
            ...
            if let Some(o) = cv.get_game_object_mut(&vname) {
                ...
                let rect_y      = (ey + rect_top_mm - mm_scroll_px).max(ey).min(ey + eh - rect_h);
                let hovered     = !is_sb_drag && mouse_pos.map(|(mx, my)| {
                    mx >= mmx && mx < sb_x && my >= ey && my <= ey + eh
                }).unwrap_or(false);
                ...
                if size_changed || hover_changed || drag_changed {
                    o.set_image(outline_rect(MINIMAP_W, rect_h, quartz::Color(150, 170, 220, 120), hovered));
