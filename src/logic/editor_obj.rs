    ...
    cv.on_update(move |cv| {
        ...
        if { *max_line_width_u.get() } <= 0.0 {
            ...
            if let Some(longest) = st.lines.iter().max_by_key(|l| l.chars().count()) {
                let t =
                    build_colored_text(std::slice::from_ref(longest), &code_font_u, &cfg, &theme);
        ...
        let (cursor_x, cursor_y) =
            cursor_position(cur_row, cur_col, slice_start, text_top, code_x, hs, cw, lh);
        let in_view = cursor_in_view(
            cursor_x, cursor_y, code_x, ex, ey, ew, eh, RIGHT_PAD, cfg.text_y,
        );
