...
use crate::constants::*;
use crate::editor::highlight::{build_gutter_slice, build_text_slice};
use crate::editor::viewer::{show_image_mode, show_text_mode, update_image_obj, FileMode};
    ...
    cv.on_update(move |cv| {
        ...
        // ── Horizontal scroll physics ─────────────────────────────────────────
        {
            ...
            if h_vel.abs() > 0.1 {
                ...
                let new_vel = if next <= 0.0 || next >= h_max {
                    0.0
                } else {
                    h_vel * cfg.scroll_friction
                };
            ...
            if *dragging_u.get() {
                if let Some((mx, my)) = cv.mouse_position() {
                    ...
                    if !st.lines.is_empty() {
                        let row = {
                            let row_float =
                                ((my_c - ey - cfg.text_y + gs) / lh).floor().max(0.0) as usize;
                            row_float.min(st.lines.len().saturating_sub(1))
                        };
                        let col = {
                            (((mx_c - ex - cfg.text_x + hs) / cw).floor().max(0.0) as usize)
        ...
        // ── Text + gutter slices ──────────────────────────────────────────────
        {
            ...
            if last_visible > slice_start {
                ...
                let cur_max = { *max_line_width_u.get() };
                if text_w > cur_max {
                    *max_line_width_u.get_mut() = text_w;
                }
