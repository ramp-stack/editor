...
fn line_to_text(
    ...
    if line.is_empty() {
        return Text::new(
            vec![Span::new(
                " ".into(), MINIMAP_FS, Some(MINIMAP_LH),
                font.clone(), Color(0, 0, 0, 0), 0.0,
            )],
    ...
    for ch in line.chars() {
        ...
        if c.0 != run_color.0 || c.1 != run_color.1 || c.2 != run_color.2 {
            ...
            if !text.is_empty() {
                spans.push(Span::new(
                    text.to_string(), MINIMAP_FS, Some(MINIMAP_LH),
                    font.clone(), run_color, 0.0,
                ));
            }
            run_start = byte_off;
            run_color = c;
    ...
    if !tail.is_empty() {
        spans.push(Span::new(
            tail.to_string(), MINIMAP_FS, Some(MINIMAP_LH),
            font.clone(), run_color, 0.0,
        ));
    }
